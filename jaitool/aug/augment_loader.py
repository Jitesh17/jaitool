...
from jaitool.draw.drawing_utils import draw_bbox, draw_keypoints, draw_mask_contour

from jaitool.structures import BBox, Segmentation, Polygon, Keypoint2D, Keypoint2D_List
...
# from .aug_utils import do_aug, custom_aug, smart_aug_getter, sometimes, bbox_based_aug_getter
from .aug_visualizer import AugVisualizer, aug_visualizer

rarely = lambda x: iaa.Sometimes(0.10, x)
occasionally = lambda x: iaa.Sometimes(0.25, x)
sometimes = lambda x: iaa.Sometimes(0.5, x)
often = lambda x: iaa.Sometimes(0.75, x)
almost_always = lambda x: iaa.Sometimes(0.9, x)
always = lambda x: iaa.Sometimes(1.0, x)
...
class AugmentedLoader(DatasetMapper):
    ...
    def mapper(self, dataset_dict, train_type: str='kpt'):
        ...
        imgaug_bboxes = BoundingBoxesOnImage(bounding_boxes=[], shape=image.shape)
        ...
        for ann in ann_dict.annotations:
            ...
            if len(ann.keypoints.to_imgaug(img_shape=image.shape).keypoints) != len(ann_dict.annotations[0].keypoints):
                printj.red(f'len(ann.keypoints.to_imgaug(img_shape=image.shape).keypoints) == {len(ann.keypoints.to_imgaug(img_shape=image.shape).keypoints)} != {len(ann_dict.annotations[0].keypoints)} == len(ann_dict.annotations[0].keypoints)')
                raise Exception
            imgaug_kpts.keypoints.extend(ann.keypoints.to_imgaug(img_shape=image.shape).keypoints)
            ...
            if ann.segmentation.to_imgaug(img_shape=image.shape).polygons is None:
                printj.red(f'ann.segmentation.to_imgaug(img_shape=image.shape).polygons is None')
                printj.red(f'ann.segmentation:\n{ann.segmentation}')
                raise Exception
            seg_len_list.append(len(ann.segmentation))
            

            imgaug_polys.polygons.extend(ann.segmentation.to_imgaug(img_shape=image.shape).polygons)
        if len(imgaug_polys.polygons) > 0:
            if num_kpts > 0:
                image, imgaug_kpts_aug, imgaug_polys_aug = seq_aug_for_seg(image=image, keypoints=imgaug_kpts, polygons=imgaug_polys)
            else:
                image, imgaug_polys_aug = seq_aug_for_seg(image=image, polygons=imgaug_polys)
                imgaug_kpts_aug = None
            imgaug_bboxes_aug = None
        else:
            if num_kpts > 0:
                image, imgaug_kpts_aug, imgaug_bboxes_aug = seq_aug_for_no_seg(image=image, keypoints=imgaug_kpts, bounding_boxes=imgaug_bboxes)
            else:
                image, imgaug_bboxes_aug = seq_aug_for_no_seg(image=image, bounding_boxes=imgaug_bboxes)
                imgaug_kpts_aug = None
            imgaug_polys_aug = None

        kpts_aug0 = Keypoint2D_List.from_imgaug(imgaug_kpts=imgaug_kpts_aug) if num_kpts > 0 else Keypoint2D_List()
        kpts_aug_list = kpts_aug0.to_numpy(demarcation=True)[:, :2].reshape(num_ann, num_kpts, 2) if num_kpts > 0 else []
        ...
        if imgaug_polys_aug is not None and imgaug_bboxes_aug is None:
            poly_aug_list = [Polygon.from_imgaug(imgaug_polygon) for imgaug_polygon in imgaug_polys_aug.polygons]
            poly_aug_list_list = unflatten_list(poly_aug_list, part_sizes=seg_len_list)
            seg_aug_list = [Segmentation(poly_aug_list) for poly_aug_list in poly_aug_list_list]
            ...
            for i in range(len(bbox_aug_list)):
                kpt_points_aug = [kpt_aug.point for kpt_aug in kpts_aug_list[i]] if num_kpts > 0 else []
                kpt_points_aug_contained = [kpt_point_aug.within(bbox_aug_list[i]) for kpt_point_aug in kpt_points_aug]
                if len(kpt_points_aug) > 0:
                    if not np.any(np.array(kpt_points_aug_contained)):
                        printj.red(f"Keypoints not contained in corresponding bbox.")
                    elif not np.all(np.array(kpt_points_aug_contained)):
                        pass
                    else:
                        break
        elif imgaug_polys_aug is None and imgaug_bboxes_aug is not None:
            bbox_aug_list = [BBox.from_imgaug(bbox_aug) for bbox_aug in imgaug_bboxes_aug.bounding_boxes]
        ...
        for item in dataset_dict["annotations"]:
            if 'keypoints' in item and num_kpts == 0:
                ...
                item['keypoints'] = np.array(item['keypoints']).reshape(-1, 3).tolist()
            annots.append(item)
    ...
    def visualize_aug(self, dataset_dict: dict):
        ...
        vis_img = image.copy()
        bbox_list = [BBox.from_list(vals) for vals in dataset_dict["instances"].gt_boxes.tensor.numpy().tolist()]
        seg_list = [Segmentation([Polygon.from_list(poly.tolist(), demarcation=False) for poly in seg_polys]) for seg_polys in dataset_dict["instances"].gt_masks.polygons] if hasattr(dataset_dict['instances'], 'gt_masks') else []
        kpts_list = [Keypoint2D_List.from_numpy(arr, demarcation=True) for arr in dataset_dict["instances"].gt_keypoints.tensor.numpy()] if hasattr(dataset_dict["instances"], 'gt_keypoints') else []
        if self.show_aug_seg:
            for seg in seg_list:
                # vis_img = draw_segmentation(img=vis_img, segmentation=seg, transparent=True)
                vis_img = draw_mask_contour(img=vis_img, mask_bool=seg, transparent=True, alpha = 0.3)
        for bbox in bbox_list:
            vis_img = draw_bbox(img=vis_img, bbox=bbox)
        for kpts in kpts_list:
            vis_img = draw_keypoints(img=vis_img, keypoints=kpts.to_numpy(demarcation=True)[:, :2].tolist(), radius=6)
    ...
    def __call__(self, dataset_dict) -> dict:
        ...
        result = copy.deepcopy(dataset_dict)  # it will be modified by code below
